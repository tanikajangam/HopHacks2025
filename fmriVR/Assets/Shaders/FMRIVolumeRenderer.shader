Shader "Custom/FMRIVolumeRenderer"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // Sample the 3D texture - use red channel for single-channel data
                float value = tex3D(_VolumeTexture, uvw).r;
                
                // Normalize the value between min and max
                float normalizedValue = saturate((_MaxValue - _MinValue) > 0.001 ? 
                    (value - _MinValue) / (_MaxValue - _MinValue) : 0.0);
